...
ops.aligntime = aligntime;
ops.plot_ch = neuron_frontal_label_list(session_sub_i);
single_unit_rastersdf_figure(spikes,ops)

burst_out = [];
for trial_i = 1:length(raster.(neuron_frontal_label_list{session_sub_i}){1})

    [oStruct] = poissBurst(raster.(neuron_frontal_label_list{session_sub_i}){trial_i}, -1000, 5000);
    try
        burst_out(trial_i,1) = oStruct.bobT(find(oStruct.bobT > 0 & oStruct.bobT < 500,1,'first'));
    catch
        burst_out(trial_i,1) = NaN;
    end
end
...
figuren; hold on
for trial_i = 1:length(raster.(neuron_frontal_label_list{session_sub_i}){1})
    scatter(raster.(neuron_frontal_label_list{session_sub_i}){trial_i}, trial_i*ones(1,length(raster.(neuron_frontal_label_list{session_sub_i}){trial_i}))', 'k','Marker','|')
    scatter(burst_out(trial_i), trial_i', 'r','filled','Marker','o')
end
